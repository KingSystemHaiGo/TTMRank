...
完整变化页默认仍使用可读、可复制、可键盘操作的 DOM 列表。用户显式切换到
“变化图谱”后才加载自托管 PixiJS 包：横轴表示时间，四条泳道表示排名、进出榜、
评分和榜单覆盖，节点大小表示重要度，方向/颜色共同表示正负变化。ticker 默认停止，
只在尺寸、筛选或选择变化时重绘；切回列表立即销毁 renderer 与 WebGL context。
省流量、硬件并发不超过 2 或 WebGL 不可用时不下载 PixiJS，继续显示重点事件 DOM。
