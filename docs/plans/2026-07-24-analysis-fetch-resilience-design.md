...
Unit tests cover both default transient retries and the new timeout opt-out. Analysis data-client tests continue to prove that the embedded made-only scope performs no request and that full-site scope fetches exactly one immutable artifact. The built-site performance suite must still prove zero default data requests. Live Playwright verification checks one full-site request, a successful switch to 996 games, no console errors, and unchanged desktop/mobile first views.
