...
export function renderBoards(container, boards, context, openDetail) {
  ...
  BOARD_DEFS.forEach(([key,title,description,tone]) => {
    ...
    list.forEach((game,index)=>{
      ...
      const row=element('div',{className:'game-row',attrs:{role:'button',tabindex:'0'},children:[element('div',{className:`rank ${index<3?'top':''}`,text:index+1}),createGameIcon(game,{proxyEndpoint:window.TTMRANK_ICON_PROXY||''}),element('div',{className:'game-main',children:[element('div',{className:'game-title',text:game.title}),element('div',{className:'game-meta',text:`${dateTime(game.released_at)} · ${age(metric?.age_hours)} · ${(game.tags||[]).filter(tag=>tag!=='TapTap制造').slice(0,2).join(' / ')||'暂无类型标签'}`})]}),element('div',{className:'game-value',children:[document.createTextNode(value),element('small',{text:label})]})]});
