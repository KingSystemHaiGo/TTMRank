...
export function renderDrawer(content, game, metric, appearances) {
  ...
  const stats=[['当前热度',compactNumber(game.heat)],['日均热度',compactNumber(metric?.heat_per_day_lifetime)],['评分',decimal(game.score)],['上线时长',age(metric?.age_hours)],['榜单覆盖',metric?.chart_coverage||appearances.length],['平台覆盖',metric?.platform_coverage||new Set(appearances.map(row=>row.platform)).size]];
